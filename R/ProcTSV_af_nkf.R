...
##'
##' @param File_path a character vector for a N-triple (NT) file (.nt) or TSV file.
##'
##' @description This function provide a Processing after the conversion from
##' their unicode (Escape character) to Multi-Byte Character.
##'
##' @return Rds
##' @author Satoshi Kume
##' @export ProcTSV_af_nkf
##'
##' @importFrom magrittr %>%
...
##' ProcTSV_af_nkf(File_path)
##'
##' }
##'

ProcTSV_af_nkf <- function(File_path){
if(!any(grepl(".nt$", File_path), grepl(".tsv$", File_path))){
...
Dat <- data.frame(readr::read_tsv(File_path,
                                  col_names = FALSE, show_col_types = FALSE))
}
...
saveRDS(Dat, file = paste0(sub("_df_nkf.tsv$", "", sub(".nt$", "", File_path)), "_df.Rds"))
